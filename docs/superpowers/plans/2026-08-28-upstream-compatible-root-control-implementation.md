...
**Interfaces:**
- Produces: `buildRootPayloadManifest(innerPath string, dsmVersion int) ([]byte, error)`.
...
- Records installed modes, including package scripts as `0755`, `conf/resource` as `0600`, privilege templates as `0644`, and inner target file modes from `package.tgz`.
...
outer package scripts, `conf/resource`, `conf/PKG_DEPS`, and both immutable
